...
img = cv2.imread(
    '/nobackup/users/mos/bbc.knmi.nl/MEMBERS/knmi/datatransfer/mobotix/vectrontest/2018/05/03/06/m180503065237523.jpg')
...
for i, file in enumerate(tqdm(sorted(os.listdir(path)))):
    ...
    camera.date = str(year) + '/' + str(month) + '/' + str(day) + ' ' + str(hour - 2) + ':' + str(minute) + ':' + str(
        second)
    ...
    if filename.endswith(".jpg"):
        ...
        maskValue, threshold, normalizedRatioBR, flatNormalizedRatioBRNoZeros, st_dev = setHYTAThreshold(masked, xres,
                                                                                                         yres)
